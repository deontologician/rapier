...
#[derive(Debug)]
pub(crate) struct BallMultibodyVelocityConstraint {
    velocity_constraint: BallVelocityConstraint,
}

impl BallMultibodyVelocityConstraint {
    pub fn from_params(
        ...
        let lhs1 = if let Some(multibody1) = multibody1 {
            multibody.fill_jacobians_generic()
        } else {
            rb_mprops1
        ...
        let lhs2 = if let Some(multibody2) = multibody2 {
            multibody.fill_jacobians_generic()
        } else {
            rb_mprops2
                .effective_world_inv_inertia_sqrt
        ...
        if motor_max_impulse > 0.0 {
            ...
            if stiffness != 0.0 || damping != 0.0 {
                motor_inv_lhs = if keep_lhs {
                    let ii1 = if let Some(multibody1) = multibody1 {
                        multibody1.fill_jacobians();
                    } else {
                        rb_mprops1.effective_world_inv_inertia_sqrt.squared();
                    };
                    let ii2 = if let Some(multibody2) = multibody2 {
                        multibody2.fill_jacobians();
                    } else {
        ...
        BallMultibodyVelocityConstraint {
            joint_id,
